...
export function ActivityFeed({ activities, compact = false, mode = "ticker" }: { activities: PublicActivity[]; compact?: boolean; mode?: "ticker" | "explore" }) {
  if (mode === "explore") return <div className="activity-explorer-feed" aria-label="Recent Atelier activity">{activities.length ? activities.map((activity) => <Link key={activity.id} href={activity.href} className="activity-explorer-card"><span className="landing-activity-avatar" aria-hidden="true">{activity.actor.slice(0, 1).toUpperCase()}</span><span><small>@{activity.actor} · {relativeTime(activity.time)}</small><strong>{activity.action} <em>{activity.subject}</em></strong></span><b>↗</b></Link>) : <div className="activity-explorer-empty"><span>✦</span><p>Collector activity will appear here as the community grows.</p><Link href="/me/perfumes/new">Share your first perfume</Link></div>}</div>;
  if (compact) return <aside className="landing-activity landing-activity--compact" aria-label="Recent Atelier activity"><div className="landing-activity-heading"><div><p className="eyebrow">AROUND ATELIER</p><h2>Fresh activity.</h2></div></div>{activities.length ? <div className="landing-activity-compact-list">{activities.slice(0, 4).map((activity) => <Link key={activity.id} href={activity.href} className="landing-activity-card"><span className="landing-activity-avatar" aria-hidden="true">{activity.actor.slice(0, 1).toUpperCase()}</span><span><small>@{activity.actor} · {relativeTime(activity.time)}</small><strong>{activity.action} <em>{activity.subject}</em></strong></span><b>↗</b></Link>)}</div> : <p className="landing-activity-compact-empty">Activity from collectors will appear here.</p>}</aside>;
  return <section className="landing-activity" aria-label="Recent Atelier activity">
    <div className="landing-activity-heading"><div><p className="eyebrow">AROUND ATELIER</p><h2>Fresh from the community.</h2></div></div>
