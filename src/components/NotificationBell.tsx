...
export function NotificationBell() {
  ...
  return <div className="relative" ref={bellRef}>
    ...
    {open ? <div className="notification-panel"><p className="px-3 py-2 text-xs uppercase tracking-wider text-muted">Activity</p>{items.length ? items.map((item) => <Link key={item.id} href={item.href} className="block rounded-xl px-3 py-3 text-sm hover:bg-bg" onClick={() => setOpen(false)}>{item.body}</Link>) : <p className="px-3 py-4 text-sm text-muted">No activity yet.</p>}</div> : null}
  </div>;
}
