...
export default async function PerfumesPage() {
  ...
  return <div className="space-y-7"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-6"><div><p className="eyebrow">Listing studio</p><h1 className="mt-2 text-4xl">Edit perfumes</h1><p className="mt-2 text-muted">Choose a listing to update its details, pricing, or visibility.</p></div><Link className="btn" href="/me/perfumes/new">Add perfume</Link></div>{perfumes.length ? <div className="grid gap-3 sm:grid-cols-2">{perfumes.map((perfume) => <PerfumeCard key={perfume.id} perfume={perfume} href={`/me/perfumes/${perfume.id}/edit`} showStatus />)}</div> : <p className="rounded-xl border border-dashed border-line p-5 text-muted">No perfumes yet. Add your first listing to begin.</p>}</div>;
}
